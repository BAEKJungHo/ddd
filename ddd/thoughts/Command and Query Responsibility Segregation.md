...
>
> 하지만, MyBatis 를 사용하는 데이터 중심 설계를 하는 개발 회사에서는 이를 적용하기 힘듦. 그 이유는 ENTITY 에서 유효성 검사를 실시하기 위해서는 객체 그래프 참조가 가능하도록 객체지향적인 설계를 해야함. A 등록할 때 A 만 등록하면 상관 없겠지만, A 등록을 하기 위해서 A 등록하고, 그 키값으로 B 등록하고, 이력 까지 쌓아야 하고 등등.. 여러 작업을 수행해야 하는데 MyBatis 를 사용하면 오히려 DTO 를 GOD CLASS 처럼 사용하고 유효성 검사 로직을 DTO 에 넣는게 생산성 측면에서는 더 좋다.
